# experiments/q2_classification.py

import numpy as np

from src.data_loader import get_data
from src.cqg_classifier import CQGClassifier
from src.dmp_classifier import DMPClassifier


def train_test_split(X, y, test_size=0.3, seed=42):
    np.random.seed(seed)
    idx = np.random.permutation(len(X))

    n_test = int(test_size * len(X))
    test_idx = idx[:n_test]
    train_idx = idx[n_test:]

    return X[train_idx], X[test_idx], y[train_idx], y[test_idx]


def evaluate_classifier(clf, X, y, n_runs=100):
    global_scores = []
    class_scores = {c: [] for c in np.unique(y)}

    for i in range(n_runs):
        Xtr, Xte, ytr, yte = train_test_split(X, y, seed=i)
        clf.fit(Xtr, ytr)

        ypred = clf.predict(Xte)

        # acurácia global
        global_scores.append(np.mean(ypred == yte))

        # acurácia por classe
        for c in class_scores:
            idx = (yte == c)
            if np.sum(idx) > 0:
                class_scores[c].append(np.mean(ypred[idx] == yte[idx]))

    results = {
        "global": (np.mean(global_scores), np.std(global_scores))
    }

    for c in class_scores:
        results[f"class_{c}"] = (
            np.mean(class_scores[c]),
            np.std(class_scores[c])
        )

    return results



def run_for_sensors(num_sensors):
    print(f"\n--- Avaliação com {num_sensors} sensores ---")

    X, y, _ = get_data(num_sensors=num_sensors)

    def print_results(name, results):
        print(f"\n{name}")
        print(f"Global: {results['global'][0]:.4f} ± {results['global'][1]:.4f}")
        for key in sorted(results.keys()):
            if key != "global":
                print(f"{key}: {results[key][0]:.4f} ± {results[key][1]:.4f}")

    cqg_results = evaluate_classifier(CQGClassifier(), X, y)
    dmp_results = evaluate_classifier(DMPClassifier(), X, y)

    print_results("CQG", cqg_results)
    print_results("DMP", dmp_results)

    return {
        "CQG": cqg_results,
        "DMP": dmp_results
    }




def run():
    print("\n==============================")
    print(" Q2 - CLASSIFICAÇÃO")
    print("==============================")

    results_4 = run_for_sensors(4)
    results_24 = run_for_sensors(24)

    return results_4, results_24


if __name__ == "__main__":
    run()
